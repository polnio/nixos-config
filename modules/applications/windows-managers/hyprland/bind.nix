...
let
  ...
  forEachWorkspace = fn:
    ...
    (builtins.genList (x: fn (builtins.toString (x + 1))) 9);

  parseKeymap = keymap:
    with lib;
    "${optionalString keymap.super "SUPER"} ${
      optionalString keymap.control "CONTROL"
    } ${optionalString keymap.shift "SHIFT"} ${
      optionalString keymap.alt "ALT"
    }, ${keymap.key}, exec, ${keymap.command}";
...
in {
  hm.wayland.windowManager.hyprland.settings = {
    ...
    ]) mvts)) ++ builtins.map parseKeymap
      (builtins.filter (x: x != null) config.settings.keymaps);
